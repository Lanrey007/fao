...
<body style="background-color: white;">
    ...
    $seluser=mysqli_query($db, "SELECT username, password FROM admin WHERE username='".$userid."'");
    if ($count1=mysqli_num_rows($seluser)<1){
        
        $error="Username enterd is not associated with ".$sitetitle.'<br/><br/>';
    
    }

    else {
        
      $getAccT=mysqli_fetch_array(mysqli_query($db, "SELECT username,email,phone,password,sid FROM admin WHERE username='".$userid."'"));
